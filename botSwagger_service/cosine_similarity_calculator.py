...
class CosineSimilarityCalculator:
    def __init__(self, original_token_dict, updated_token_dict, nlu_dict) -> None:
        self.original_token_dict = original_token_dict
        self.updated_token_dict = updated_token_dict
        self.nlu_dict = nlu_dict

    # 定義如何取得文件權重的 dict
    def _get_sentence_weight_dict(self, token_dict, query_sentence_id) -> dict:
        sentence_weight_dict = dict()
        for token in token_dict:
            sentence_weight_dict[token] = 0
            # 跳過 statistics_node
            for document_node in token_dict[token][1:]:
                if query_sentence_id == document_node['id']:
    ...
    def get_tfidf_similarity_dict(self, query_sentence_id) -> dict:
        ...
        query_sentence_weight_dict = self._get_sentence_weight_dict(self.updated_token_dict, query_sentence_id)
        query_sentence_vector = self._get_sentence_vector(query_sentence_weight_dict)
        # print('[query_sentence_vector]', query_sentence_vector)
        # 取得 similarity_dict
        for sentence_id in self.nlu_dict:
            corpus_sentence_weight_dict = self._get_sentence_weight_dict(self.updated_token_dict, sentence_id)
            corpus_sentence_vector = self._get_sentence_vector(corpus_sentence_weight_dict)
